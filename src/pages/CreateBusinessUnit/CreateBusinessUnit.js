...
const CreateBusinessUnit = () => {
  ...
  const onSubmit = () => {
    if (validator.allValid()) {
      const updatedInputFields = inputFields.map((element) => {
        let localNumber = formatPhoneNumber(element.contact_person_number);
        localNumber = localNumber.replace(/^0+/, "");
        const interNational = formatPhoneNumberIntl(
          element.contact_person_number
        );
        const localNumberArray = interNational.split(" ");

        element.contact_person_number = localNumber;
        element.contact_person_country_code = localNumberArray[0];
                          ...
                          <div className="mb-4">
                            <Grid container spacing={2.5}>
                              ...
                              <Grid item md={3} sm={6} xs={12}>
                                ...
                                />
                                {validator.message(`pincode`, input.pincode, [
                                  "required",
                                  { max: "125" },
                                ])}
                              </Grid>
