...
const ClientCreation = (props) => {
  ...
  const onSubmit = () => {
    if (validator.allValid()) {
      ...
      let localNumber = formatPhoneNumber(mobileNumber);
      localNumber = localNumber.replace(/^0+/, "");
      const interNational = formatPhoneNumberIntl(mobileNumber);
      const localNumberArray = interNational.split(" ");

      let adminNumber = formatPhoneNumber(adminMobile);
      adminNumber = adminNumber.replace(/^0+/, "");
      const adminInternational = formatPhoneNumberIntl(adminMobile);
      const adminArray = adminInternational.split(" ");

      const client = {
        organization: {
          name: clientName,
          tan: tanNumber,
          country_code: localNumberArray[0],
        ...
        user: {
          ...
          password: "admin123",
          mobile: adminNumber,
          mobile_country_code: adminArray[0],
        },
      };
